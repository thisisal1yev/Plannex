...
function StatCard({ title, value, sub, icon: Icon, trend, trendUp }: StatCardProps) {
  return (
...
function RevenueChart({ data }: { data: { month: string; revenue: number }[] }) {
  return (
    <div className="bg-card border border-border rounded-xl p-5">
      <h3 className="text-sm font-semibold text-foreground mb-4">Total Revenue</h3>
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
          ...
          <Tooltip
            formatter={(value: number) => [formatUZS(value), 'Revenue']}
            contentStyle={{
              background: 'hsl(var(--card))',
              border: '1px solid hsl(var(--border))',
              borderRadius: 8,
  ...
  const columns: TableColumn<AdminPendingEvent>[] = [
    {
      ...
      render: (e) => (
        <div className="flex items-center gap-3">
          {e.bannerUrl ? (
            <img src={e.bannerUrl} alt="" className="w-8 h-8 rounded object-cover shrink-0" />
          ) : (
          ...
          <div className="flex flex-col">
            <span className="font-medium text-foreground text-sm">{e.title}</span>
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Real-time platform analytics and event management</p>
        </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon={Clock}
          sub="Awaiting review"
          trend={data && data.pendingEvents > 0 ? `${data.pendingEvents} need action` : undefined}
          trendUp={false}
        />
        <StatCard
          title="Total Events"
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold text-foreground">Recent Event Submissions</h2>
