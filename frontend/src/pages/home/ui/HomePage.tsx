  ...
  return (
    <div className="bg-navy text-cream">
      ...
      <section
        ...
        style={{
          background: 'radial-gradient(ellipse 90% 55% at 50% -5%, rgba(201,150,58,0.09) 0%, transparent 68%), #0C1520',
      ...
      >
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 py-28 text-center w-full">
          ...
          <p
            ...
            style={{ fontSize: '18px', lineHeight: 1.75, maxWidth: '520px', margin: '0 auto 44px' }}
          >
          ...
          <div className="lp-a lp-d5 grid grid-cols-2 sm:grid-cols-4 gap-3 max-w-[580px] mx-auto">
            {STATS.map((s) => (
              ...
              >
                <div className="lp-serif text-gold-light font-bold leading-none" style={{ fontSize: '30px' }}>
                  {s.v}
                </div>
                <div className="text-clay text-[12px] mt-[5px] tracking-[0.04em]">{s.l}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div
          ...
          style={{ background: 'linear-gradient(to bottom, transparent, #0C1520)' }}
        />
      </section>
        ...
        <div className="max-w-[1100px] mx-auto">
          <div className="flex items-end justify-between flex-wrap gap-4 mb-12">
            <div>
              ...
              <h2 className="lp-serif text-cream" style={{ fontSize: 'clamp(32px,5vw,46px)', fontWeight: 700, lineHeight: 1.1 }}>
                Tadbir turini tanlang
              </h2>
            </div>
            <Link to="/events" className="text-gold text-sm no-underline hover:opacity-70 transition-opacity">
              Barcha tadbirlar →
            </Link>
          </div>
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {CATS.map((c) => (
              ...
              >
                ...
                <span className="text-[12px] font-medium text-cream text-center leading-[1.3]">{c.l}</span>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-[1100px] mx-auto">
          <div className="flex items-end justify-between flex-wrap gap-4 mb-12">
            <div>
              ...
              <p className="text-clay text-[15px] mt-2">O'zbekistonning dolzarb tadbirlari</p>
            </div>
            <Link to="/events" className="text-gold text-sm no-underline">Barcha tadbirlar →</Link>
          </div>
          {eventsLoading
            ? <Spinner />
      ...
      {/* ════════════════════════════════ HOW IT WORKS ══════════════════════ */}
      <section id="how-it-works" className="py-[88px] px-6 bg-navy-3 border-y border-gold/15">
        <div className="max-w-[1100px] mx-auto">
          <div className="text-center mb-[72px]">
            <Label text="Jarayon" />
            <h2 className="lp-serif text-cream" style={{ fontSize: 'clamp(32px,5vw,48px)', fontWeight: 700 }}>
              Bu qanday ishlaydi
            </h2>
            ...
            {STEPS.map((s, i) => (
              <div key={s.n} className="relative">
                <div
                  ...
                  style={{ fontSize: '88px', fontWeight: 700, lineHeight: 1, color: 'transparent', WebkitTextStroke: '1px rgba(201,150,58,0.18)' }}
                >{s.n}</div>
                {i < 2 && (
                  <div className="hidden md:block absolute text-[22px] z-10" style={{ top: '32px', right: '-28px', color: 'rgba(201,150,58,0.15)' }}>→</div>
                )}
                <div className="w-9 h-0.5 bg-gold opacity-55 mb-4" />
                <h3 className="text-[19px] font-semibold text-cream mb-[10px]">{s.t}</h3>
        ...
        <div className="max-w-[1100px] mx-auto">
          <div className="flex items-end justify-between flex-wrap gap-4 mb-12">
            <div>
              ...
              <p className="text-clay text-[15px] mt-2">Tadbirlaringiz uchun eng yaxshi joylar</p>
            </div>
            <Link to="/venues" className="text-gold text-sm no-underline">Barcha maydonlar →</Link>
          </div>
          {venuesLoading
            ? <Spinner />
            : (
        ...
        <div className="max-w-[1100px] mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {FEATS.map((f) => (
              ...
              >
                <div
                  className="text-[26px] mb-[18px] inline-flex items-center justify-center w-[52px] h-[52px] rounded-xl bg-gold/6 border border-gold/15"
                >{f.e}</div>
                <h3 className="text-[16px] font-semibold text-cream mb-2">{f.t}</h3>
      ...
      {/* ════════════════════════════════ PRICING ═══════════════════════════ */}
      <section id="pricing" className="py-[88px] px-6 bg-navy-3 border-t border-gold/15">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {PLANS.map((p) => (
              <div
                ...
                style={{
                  padding: '32px 24px',
                  border: p.hot ? '1px solid #C9963A' : '1px solid rgba(201,150,58,0.15)',
                  background: p.hot
              ...
              >
                {p.hot && (
                  <div
                    ...
                    style={{
                      ...
                      color: '#0C1520', padding: '4px 16px', borderRadius: '100px',
                    }}
                  >
                    Eng mashhur
                  </div>
                )}
                <p className="text-[11px] text-clay tracking-[0.12em] uppercase mb-[10px]">{p.n}</p>
                <div className="flex items-baseline gap-1 mb-1">
                  <span className="lp-serif font-bold leading-none" style={{ fontSize: '38px', color: p.hot ? '#E8C06A' : '#F0E8D4' }}>{p.p}</span>
                  {p.per && <span className="text-[13px] text-clay">{p.per}</span>}
                </div>
                <p className="text-[13px] text-clay mb-6">{p.desc}</p>
                <ul className="list-none p-0 m-0 mb-7 flex flex-col gap-[10px] flex-1">
                  {p.fs.map((f) => (
                    <li key={f} className="flex items-start gap-[10px] text-sm text-cream">
                      <span className="text-gold font-bold text-[12px] mt-[2px] shrink-0">✓</span>
                      {f}
                    </li>
                  ))}
                </ul>
                <Link
                  ...
                  style={{
                    padding: '12px',
                    background: p.hot ? 'linear-gradient(135deg, #C9963A, #9E7220)' : 'transparent',
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col gap-[14px]">
              {[
                ...
                >
                  ...
                  <div>
                    <p className="text-[12px] font-semibold text-gold-light tracking-[0.06em] uppercase mb-[3px]">{x.t}</p>
            ...
            >
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] text-gold tracking-widest uppercase mb-[7px]">Ism</label>
                  <input className="lp-input" type="text" placeholder="Ismingiz" />
                </div>
                <div>
                  <label className="block text-[11px] text-gold tracking-widest uppercase mb-[7px]">Email</label>
                  <input className="lp-input" type="email" placeholder="email@example.com" />
                </div>
              </div>
              <div>
                <label className="block text-[11px] text-gold tracking-widest uppercase mb-[7px]">Mavzu</label>
                <input className="lp-input" type="text" placeholder="Qanday yordam bera olamiz?" />
              </div>
              <div>
                <label className="block text-[11px] text-gold tracking-widest uppercase mb-[7px]">Xabar</label>
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <h2
                ...
                style={{ fontSize: 'clamp(34px, 6vw, 62px)', fontWeight: 700, lineHeight: 1.08 }}
              >
                Tadbir boshlashga{' '}
