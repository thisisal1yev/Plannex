        ...
        <div className="grid grid-cols-1 gap-10 mb-12 sm:grid-cols-4">
          ...
          <div className="flex flex-col gap-3.5">
            <Link to="/" className="no-underline inline-flex items-center">
              <span className="font-bold text-[17px] text-cream tracking-[-0.01em]">Planner</span>
              <span className="font-bold text-[17px] text-gold tracking-[-0.01em]"> AI</span>
            </Link>
          ...
          {COLS.map(({ title, links }) => (
            <div key={title} className="flex flex-col gap-3.5">
              ...
              <ul className="list-none p-0 m-0 flex flex-col gap-2.5">
                {links.map(({ label, to }) => (
                  <li key={label}>
                    <Link to={to} className="text-[13px] text-cream/42 no-underline leading-none transition-colors duration-200 hover:text-gold">
                      {label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="border-t border-t-gold/10 pt-6 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-5">
            {[
              ...
              <Link key={to} to={to} className="text-[13px] text-cream/42 no-underline leading-none transition-colors duration-200 hover:text-gold">
                {label}
              </Link>
            ))}
          </div>
